...
//
// Introspection is brokered by the vulos-cloud control plane ("CP"). Mail (and
...
//
// This package imports nothing from the rest of mail: it is the wire seam, so
...
const (
	...
	// Bearer credential. A token without this prefix is treated as a session
	// cookie / webmail token.
	KeyPrefix = "vk_"

	// ProductMail is the product scope a key MUST carry to use the mail /v1
	...
	// When unset the key path is disabled and mail falls back to session-only
	// auth (self-host unchanged). This is the SAME env the cloud billing seam uses.
	EnvCPBaseURL = "VULOS_CP_BASE_URL"

	// EnvCPToken is the service token mail presents to the CP on the
	// introspection call. Optional (the CP may authorise by network instead).
