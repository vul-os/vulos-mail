...
func TestIntrospect_ValidKey(t *testing.T) {
	srv, _ := mockCP(t, func(req introspectRequest) Result {
		return Result{Valid: true, Account: "alice@vulos.org", Scopes: []string{"mail.read"}, Products: []string{"mail", "office"}}
	})
	...
	if !res.HasProduct(ProductMail) {
		t.Fatal("expected mail product")
	}
	if res.HasProduct("talk") {
		t.Fatal("did not expect talk product")
	}
	if !res.HasScope("mail.read") {
		t.Fatal("expected mail.read scope")
	}
}
...
func TestIntrospect_CachesResults(t *testing.T) {
	srv, calls := mockCP(t, func(req introspectRequest) Result {
		return Result{Valid: true, Account: "bob@vulos.org", Products: []string{"mail"}}
	})
	intro := NewIntrospectorWithClient(Config{BaseURL: srv.URL}, srv.Client())
...
func TestIntrospect_CacheExpires(t *testing.T) {
	srv, calls := mockCP(t, func(req introspectRequest) Result {
		return Result{Valid: true, Account: "carol@vulos.org", Products: []string{"mail"}}
	})
	intro := NewIntrospectorWithClient(Config{BaseURL: srv.URL}, srv.Client())
...
func TestIntrospect_SendsServiceToken(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(Result{Valid: true, Products: []string{"mail"}})
	}))
	t.Cleanup(srv.Close)
...
func TestIntrospect_CachesInvalidResults(t *testing.T) {
	// Both valid AND invalid results must be cached to protect against hot
	// invalid-key floods hammering the CP.
	srv, calls := mockCP(t, func(req introspectRequest) Result {
	...
	for i := 0; i < 3; i++ {
		if _, err := intro.Introspect(context.Background(), "vk_invalid"); err != nil {
			t.Fatalf("introspect %d: %v", i, err)
		}
	}
	if got := atomic.LoadInt32(calls); got != 1 {
		t.Fatalf("expected 1 CP call for invalid key (cached), got %d", got)
	}
}
